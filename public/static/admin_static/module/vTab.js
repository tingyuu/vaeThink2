...
/**删除缓存中的tabMenu**/
let removeTabMenu = function (vTab, callback) {
...
layui.define(["element", "jquery"], function (exports) {
	 ...
	 * @returns {vTab}
	 */
	vTab.prototype.init = function (option) {
		var _this = this; //这个this就是代表vTab（因为prototype的原因）
	...
	//生成左侧菜单
	var temp = "";
	vTab.prototype.navBar = function (strData) {
		var data;
	 ...
	 * @param ele 当前tab
	 */
	vTab.prototype.positionTab = function (contEle, ele) {

		var superEle = $(".v-tab");//父级元素
	...
	vTab.prototype.tab = function (e) {
		...
		element.on("tab({0})".format(filter), function (data) {
			...
			var elSuper = $(".v-tab"),//视窗元素
				elMove = $(".v-tab-title"),//移动的元素
				elTabs = $(".v-tab-title li"),//所有已存在的tab集合
	...
	vTab.prototype.tabDelete = function () {
		var that = this;
		var filter = this.tabConfig.tabFilter;
	...
	//删除缓存中的tab
	vTab.prototype.removeTabStorage = function (callback) {
		removeTabMenu(this, callback);
	};

	//添加tab页
	vTab.prototype.tabAdd = function (_thisa) {
		var that = this;
		...
		var thatTabNum = $('.v-tab-title > li').length;//当前已经打开的选项卡的数量
		var iframes = $(".v-tab-content iframe");
		...
		if (_this.attr("target") == "_blank") {
			...
			for (var i = 0; i < thatTabNum; i++) {
				if ($('.v-tab-title > li').eq(i).attr('lay-id') == tabId) {
	...
	vTab.prototype.render = function (callback) {
		var _this = this;//data
	...
	//刷新当前tab页
	vTab.prototype.refresh = function (_this, callback) {
		if (!($(_this).hasClass("refreshThis"))) {
			$(_this).addClass("refreshThis");
			$(".v-tab-content .layui-show").find("iframe")[0].contentWindow.location.reload(true);
			if(typeof callback == "function"){
				callback(vTab);
			}
			setTimeout(function () {
	 ...
	 * 3：代表关闭所有标签页
	 */
	vTab.prototype.tabClose = function (num) {
		num = num || 1;
		num = num * 1;//强制转换成数字
		var that = this;
		let openTabs = $('.v-tab-title > li strong[lay-id]').not('strong[is-close=false]'),//获取已打开的tab元素(除开不会被关闭的导航)
			thatLayID = $('.v-tab-title > li.layui-this strong[lay-id]').not('strong[is-close=false]').attr("lay-id") || '';//获取当前打开的tab元素ID(除开不会被关闭的导航)
	 ...
	 *      为leftmax:跑到最左边
	 */
	vTab.prototype.navMove = function (moveId, that) {
		var superEle = $(".v-tab"),//父级元素
			contEle = $(".v-tab-title"),//tab内容存放的父元素
			superWidth = parseInt(superEle.width()) - 40 * 3,//可显示的宽度
	...
	exports("vTab", function (option) {
		if (parent.objvTab) {
			return parent.objvTab;
		} else {
			return new vTab().init(option);
		}
	});
